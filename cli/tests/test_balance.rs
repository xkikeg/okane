...
fn balance_filter_exact(
    ...
    golden_path.push(filename);
    assert!(
        golden_path.set_extension("golden.balance.account_filter_exact.txt"),
        "failed to set extension .ledger to input {}",
        input.display()
    );
    log::info!("golden_path: {}", golden_path.display());
    ...
    let result = assert_cmd::Command::new(&*testing::BIN_PATH)
        .args([
            "balance".as_ref(),
            input.as_os_str(),
            "--account-filter=exact".as_ref(),
            "Assets:Banks:あおによし".as_ref(),
            "Liabilities:Cards:Card X".as_ref(),
        ])
        .assert()
        .success();
...
fn balance_filter_regex(
    ...
    golden_path.push(filename);
    assert!(
        golden_path.set_extension("golden.balance.account_filter_regex.txt"),
        "failed to set extension .ledger to input {}",
        input.display()
    );
    log::info!("golden_path: {}", golden_path.display());
