...
//! place that owns the mutable [`LoadedImport`] the pure state machine
//! cannot touch.
...
/// Executes a [`Command`] returned from [`ReviewApp::update`]. The only
/// impure step in the loop — it owns the `&mut LoadedImport` the pure state
/// machine cannot touch.
fn fulfill(app: &mut ReviewApp, loaded: &mut LoadedImport, cmd: Command) -> anyhow::Result<()> {
    match cmd {
        Command::AcceptPending { index } => {
            loaded.txns[index].clear_state(ClearState::Uncleared);
            let preview = loaded
                .render_transaction(index)
...
mod tests {
    ...
    use crate::ui::review::app::ReviewItem;
    ...
    fn prompt_control_keys() {
        ...
        assert_eq!(key_to_message(&app, ctrl_key('g')), Some(Message::PromptCancel));
        assert_eq!(
        ...
        assert_eq!(key_to_message(&app, ctrl_key('n')), Some(Message::PromptNext));
        assert_eq!(key_to_message(&app, ctrl_key('p')), Some(Message::PromptPrev));
        assert_eq!(
