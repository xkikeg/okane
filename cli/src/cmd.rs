...
pub enum Command {
    ...
    Import(ImportCmd),
    /// Format the ledger file and all the files it includes, in place.
    #[command(alias = "fmt")]
    Format(FormatCmd),
...
impl Command {
    ...
    where
        W: std::io::Write,
    {
        // `format --check` is the only command reporting its outcome through
        // the exit code; everything else fails by returning an error.
        match self {
...
impl FormatCmd {
    ...
    where
        W: std::io::Write,
    {
        let loader = load::new_loader(self.source.clone());
        if !self.check {
            okane_core::format::format_recursively(&loader, &mut format::WriteBack)?;
            return Ok(ExitCode::SUCCESS);
        }
        let mut emitter = format::WriteDiff::new(w);
        okane_core::format::format_recursively(&loader, &mut emitter)?;
        if emitter.has_diff() {
            Ok(ExitCode::FAILURE)
        } else {
            Ok(ExitCode::SUCCESS)
...
impl PrimitiveFormatCmd {
        ...
        W: std::io::Write,
    {
        ...
        self.format_options().format(&mut r, w)?;
        Ok(())
    }
    ...
    fn format_options(&self) -> okane_core::format::FormatOptions {
        ...
        let mut default_commodity = CommodityDisplayOption::default();
        let mut given_formats = HashMap::new();
        for given in &self.commodity_format {
            match &given.commodity {
                None => default_commodity = given.option,
                Some(commodity) => {
                    given_formats.insert(commodity.as_str(), given.option);
                }
            }
        }
        let mut builder = DisplayContextBuilder::new().with_default_commodity(default_commodity);
...
impl FlattenCmd {
        ...
        W: std::io::Write,
    {
        ...
        // DisplayContextBuilder, the way `okane format` does.
